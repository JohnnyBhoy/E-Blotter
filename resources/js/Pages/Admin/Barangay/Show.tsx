...
export default function BarangayShow({ auth, barangay, blotters, stats, trend, filters }: ShowProps) {
    const [search, setSearch]   = useState(filters.search);
    const [remark, setRemark]   = useState(filters.remark);
    const [perPage, setPerPage] = useState(filters.per_page);

    const applyFilters = (overrides: Record<string, any> = {}) => {
        router.get(route("admin.barangay.show", barangay.id), {
            search,
            remark,
            per_page: perPage,
            ...overrides,
    ...
    return (
        <AuthenticatedLayout user={auth.user}>
            <Head title={`${barangay.brgy_name} — Blotter Details`} />
            ...
            <div className="min-h-screen bg-gray-50 dark:bg-claude-bg">
                <div className="p-6 space-y-6">
                    ...
                    <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-claude-text-muted">
                        <Link href={route("admin.barangay")} className="flex items-center gap-1 hover:text-claude-accent transition-colors">
                            <ArrowLeft className="w-3.5 h-3.5" /> Barangays
                        </Link>
                        <span>/</span>
                        <span className="text-gray-900 dark:text-claude-text font-medium">{barangay.brgy_name}</span>
                    </div>
                    ...
                    <div className="bg-white dark:bg-claude-panel rounded-xl border border-gray-100 dark:border-claude-border p-5">
                        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                            <div className="flex items-start gap-4">
                                ...
                                <div>
                                    <h1 className="text-xl font-bold text-gray-900 dark:text-claude-text">{barangay.brgy_name}</h1>
                                    <div className="flex flex-wrap gap-3 mt-1.5 text-sm text-gray-500 dark:text-claude-text-muted">
                                        <span className="flex items-center gap-1">
                                            <MapPin className="w-3.5 h-3.5" />
                                            {getCity(barangay.city_code) || "—"}, {getProvince(barangay.province_code) || "—"}
                                        </span>
                                        <span className="flex items-center gap-1">
                            ...
                            <div className="flex gap-2 flex-shrink-0">
                                <Link href={route("admin.barangay.edit", barangay.id)}
                                ...
                                <span className={`px-3 py-2 rounded-lg text-xs font-medium self-center ${barangay.email ? "bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400" : "bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400"}`}>
                                    {barangay.email ? "Active Account" : "No Account"}
                                </span>
                            </div>
                        </div>
                    </div>
