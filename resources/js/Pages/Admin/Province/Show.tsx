...
export default function ProvinceShow({ auth, province, stations, blotters, stats, trend, filters }: ShowProps) {
    const [search, setSearch]   = useState(filters.search);
    const [remark, setRemark]   = useState(filters.remark);
    const [perPage, setPerPage] = useState(filters.per_page);

    const applyFilters = (overrides: Record<string, any> = {}) => {
        router.get(route("admin.province.show", province.id), {
            search,
            remark,
            per_page: perPage,
            ...overrides,
    ...
    return (
        <AuthenticatedLayout user={auth.user}>
            <Head title={`${province.province_name} — Province Detail`} />
            ...
            <div className="min-h-screen bg-gray-50 dark:bg-claude-bg">
                <div className="p-6 space-y-6">
                    ...
                    <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-claude-text-muted">
                        <Link href={route("admin.province")} className="flex items-center gap-1 hover:text-claude-accent transition-colors">
                            <ArrowLeft className="w-3.5 h-3.5" /> Provinces
                        </Link>
                        <span>/</span>
                        <span className="text-gray-900 dark:text-claude-text font-medium">{province.province_name}</span>
                    </div>
                    ...
                    <div className="bg-white dark:bg-claude-panel rounded-xl border border-gray-100 dark:border-claude-border p-5">
                        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                            <div className="flex items-start gap-4">
                                ...
                                <div>
                                    <h1 className="text-xl font-bold text-gray-900 dark:text-claude-text">{province.province_name}</h1>
                                    <div className="flex flex-wrap gap-3 mt-1.5 text-sm text-gray-500 dark:text-claude-text-muted">
                                        <span className="flex items-center gap-1">
                                            <MapPin className="w-3.5 h-3.5" />
                                            {getProvince(province.province_code) || province.province_code}
                                        </span>
                                        {province.email && (
                                            <span className="flex items-center gap-1">
                                        ...
                                        <span className="flex items-center gap-1">
                                            ...
                                            {stats.barangays} barangay{stats.barangays !== 1 ? "s" : ""}
                                        </span>
                                    </div>
                                </div>
                            </div>
                            <div className="flex gap-2 flex-shrink-0">
                                ...
                                <span className={`px-3 py-2 rounded-lg text-xs font-medium self-center ${province.email ? "bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400" : "bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400"}`}>
                                    {province.email ? "Active Account" : "No Account"}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="bg-white dark:bg-claude-panel rounded-xl border border-gray-100 dark:border-claude-border p-5">
                            <div className="flex items-center gap-2 mb-3">
                                <Shield className="w-4 h-4 text-claude-accent" />
                                <h2 className="text-sm font-semibold text-gray-900 dark:text-claude-text">Police Stations</h2>
                                <span className="ml-auto text-xs text-gray-400 dark:text-claude-text-muted">{stations.length} total</span>
                            </div>
                            <div className="space-y-1.5 max-h-40 overflow-y-auto pr-1">
                                {stations.length === 0 ? (
                                    <p className="text-xs text-gray-400 dark:text-claude-text-muted text-center py-4">No stations in this province</p>
                                ) : stations.map(s => (
                                    <div key={s.id} className="flex items-center justify-between py-1.5 px-2 rounded-lg hover:bg-gray-50 dark:hover:bg-claude-panel-2 transition-colors">
                                        <div className="flex items-center gap-2 min-w-0">
                                            <Shield className="w-3 h-3 text-claude-accent flex-shrink-0" />
                                            <div className="min-w-0">
                                                <p className="text-xs text-gray-700 dark:text-claude-text truncate">{s.station_name}</p>
                                                <p className="text-[10px] text-gray-400 dark:text-claude-text-muted">{getCity(s.city_code) || s.city_code}</p>
                                            </div>
                                        </div>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-gray-100 dark:border-claude-border bg-gray-50/50 dark:bg-claude-panel-2/30">
                                        ...
                                        <th className="px-5 py-3 text-left text-xs font-semibold text-gray-600 dark:text-claude-text-muted">City/Municipality</th>
                                ...
                                <tbody className="divide-y divide-gray-50 dark:divide-claude-border">
                                    {blotters.data.length === 0 ? (
                                        ...
                                        <tr key={b.id} className="hover:bg-gray-50/60 dark:hover:bg-claude-panel-2/40 transition-colors">
                                            ...
                                            <td className="px-5 py-3 text-xs text-gray-700 dark:text-claude-text">
                                                {b.barangay_name || <span className="text-gray-400 dark:text-claude-text-muted">—</span>}
                                            </td>
                                            <td className="px-5 py-3 text-xs text-gray-500 dark:text-claude-text-muted">
                                                {b.city_code ? getCity(b.city_code) || b.city_code : "—"}
                                            </td>
