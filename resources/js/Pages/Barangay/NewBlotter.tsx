...
import { ArrowLeft, ArrowRight, CircleHalf, CloudUpload, Save } from "react-bootstrap-icons";
...
export default function NewBlotter({ auth }: { auth: any }) {
    ...
    const validateStep = (step: number): boolean => {
        ...
        if (step === 2) {
            ...
            if (!complainant.complainant_birth_date) {
                errors.complainant_birth_date = 'Complainant birth date is required';
            }
            if (!complainant.complainant_place_of_birth?.trim()) {
                errors.complainant_place_of_birth = 'Complainant place of birth is required';
            }
            if ([complainant.complainant_region, complainant.complainant_province, complainant.complainant_city, complainant.complainant_barangay].includes(0)) {
                errors.complainant_address = 'Complete complainant address is required';
            }
        }
        
        if (step === 3) {
            // Narrative validation
            if (!data.narrative?.trim() || data.narrative === "(Detail the narrative of the incident or event, answering the WHO, WHAT, WHERE, WHY and HOW of reporting either in English or common dialect)") {
    ...
    const handleNextStep = () => {
        if (!validateStep(currentStep)) {
            SweetAlert(`Validation Error`, 'Please fill in all required fields.', 'error', 3000);
            return;
        }
    ...
    const handleNext = () => {
        if (currentStep === 2) {
            ...
            if (data.barangay == "" || data.barangay == null)
                return SweetAlert(`Barangay is required!`, 'Unable to proceed, please provide barangay.', 'error', 2500);
            ...
            if (data.date_reported == "")
                return SweetAlert(`Date reported is required!`, 'Unable to proceed, please provide date reported.', 'error', 2500);

            if (data.time_of_report == "")
                return SweetAlert(`Time reported is required!`, 'Unable to proceed, please provide time reported.', 'error', 2500);

            if (data.date_of_incident == "")
                return SweetAlert(`Date of incident is required!`, 'Unable to proceed, please provide date of incident.', 'error', 2500);

            if (data.time_of_incident == "")
                return SweetAlert(`Time of incident is required!`, 'Unable to proceed, please provide time of incident.', 'error', 2500);

            const complainant = data.complainant_data[0];
            if (complainant.complainant_family_name == "" || complainant.complainant_first_name == "" || complainant.complainant_middle_name == "")
                return SweetAlert(`Complainant name is required!`, 'Unable to proceed, please provide Complainant name.', 'error', 2500);

            if (complainant.complainant_birth_date == "")
                return SweetAlert(`Complainant birth date is required!`, 'Unable to proceed, please provide Complainant birth date.', 'error', 2500);

            if (complainant.complainant_place_of_birth == "")
                return SweetAlert(`Complainant place of birth is required!`, 'Unable to proceed, please provide Complainant place of birth.', 'error', 2500);
    ...
    const Submit = (e: FormEvent) => {
        ...
        // Final validation
        if (!validateStep(4)) {
            SweetAlert(`Validation Error`, 'Please complete all required fields before submitting.', 'error', 3000);
            return;
        }
                                        ...
                                        <React.Fragment key={step}>
                                            <div className="flex flex-col items-center">
                                                ...
                                                <span className={`mt-3 text-xs font-medium transition-colors duration-300 ${
                                                    currentStep >= step ? 'text-gray-900 dark:text-white font-semibold' : 'text-gray-500 dark:text-gray-400'
                                                }`}>
                                                    {step === 1 && 'BRF Form'}
                                                    {step === 2 && 'Complainant'}
                                                    {step === 3 && 'Narrative'}
                                            ...
                                            {step < 4 && (
                                                <div className={`flex-1 h-1 mx-6 transition-all duration-500 rounded-full ${
                                                    currentStep > step ? 'bg-gradient-to-r from-blue-400 to-blue-500' : 'bg-gradient-to-r from-gray-200 to-gray-300'
                                                }`} />
                                            )}
                                        </React.Fragment>
                                    ))}
                    ...
                    {Object.keys(formErrors).length > 0 && (
                        <div className="bg-red-50 border border-red-100 rounded-xl p-5 shadow-sm">
                            <div className="flex items-center mb-3">
                                ...
                                <h4 className="text-red-700 font-medium">Please correct the following errors:</h4>
                            </div>
                    ...
                    <div className="flex justify-between items-center mt-8 p-6 bg-gray-50 rounded-xl shadow-sm">
                        ...
                        <div className="flex gap-3 w-full sm:w-auto">
                            ...
                            {currentStep < 4 ? (
                                ...
                                <form onSubmit={Submit} className="flex-1 sm:flex-none">
                                    <button
                                        type="submit"
