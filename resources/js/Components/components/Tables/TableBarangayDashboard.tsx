...
const TableBarangayDashboard = () => {
  // Global states
  const { hearing, settled, pending, referred, blotter, top10Cases } = useBlotterStore();
      ...
      <div className="flex flex-col">
        ...
        <div
          ...
          <div className="flex items-center justify-center p-2.5 xl:p-3 border border-1 border-solid border-slate-700">
            <p className="text-black dark:text-white">{blotter < 1000 ? blotter : `${(blotter / 1000)?.toFixed(1)}K`}</p>
          </div>
          ...
          <div className="flex items-center justify-center p-2.5 xl:p-3 border border-1 border-solid border-slate-700">
            <p className="text-black dark:text-white">{settled < 1000 ? settled : `${settled / 1000}K`}</p>
          </div>

          <div className="flex items-center justify-center p-2.5 xl:p-3 border border-1 border-solid border-slate-700">
            <p className="text-black dark:text-white">{pending < 1000 ? pending : `${pending / 1000}K`}</p>
          </div>

          <div className="flex items-center justify-center p-2.5 xl:p-3 border border-1 border-solid border-slate-700">
            <p className="text-black dark:text-white">{hearing < 1000 ? hearing : `${hearing / 1000}K`}</p>
          </div>

          <div className="hidden items-center justify-center p-2.5 sm:flex xl:p-3 border border-1 border-solid border-slate-700">
            <p className="text-black dark:text-white">{referred < 1000 ? referred : `${referred / 1000}K`}</p>
          </div>

          <div className="hidden items-center justify-center p-2.5 sm:flex xl:p-3 border border-1 border-solid border-slate-700">
            <p className="text-black dark:text-white">{others < 1000 ? others : `${others / 1000}K`}</p>
          </div>
        </div>
      </div>
