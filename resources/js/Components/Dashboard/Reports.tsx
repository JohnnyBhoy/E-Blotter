...
const Reports = ({ incidents, setSelectedReport, setShowReport }:
    { incidents: IncidentsProps, setSelectedReport: CallableFunction, setShowReport: CallableFunction }) => {
    ...
    return (
        <div className="relative overflow-x-auto shadow-md sm:rounded-lg mt-10 border border-1 border-slate-300">
            <div className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400 flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between pb-4 p-2 bg-white">
                <select name="" id="" className='rounded-lg py-1 text-slate-700 text-sm'>
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        className="block p-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search for incident..." />
                </div>
            </div>
            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    {incidents
                        ?.filter((incident) => incident.location.includes(incidentFilter)
                        ...
                        ?.map((incident: IncidentProps, i: number) => (
                            <tr key={i} className="text-xs text-gray-700 uppercase dark:bg-gray-700 dark:text-gray-400 bg-white">
                                ...
                                <td scope="row" className="dark:bg-gray-800 text-xs px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                                    📍{incident.location?.substring(0, 30)}...
                                </td>
                                <td className="px-6 py-4">
                                   {getIncident(incident.incidentTypes)}
                                </td>
                                <td className="px-6 py-4">
                                  {getStatus(incident.status)}
                                </td>
                                <td className="px-6 py-4">
                                  {incident.incident_responder ?? "⚠️ Please assign"}
                                </td>
                                ...
                                <td className="px-6 py-4">
                                    <a href="#"
                                        onClick={() => handleShowReportDetails(incident)}
                                        className="font-medium text-blue-600 dark:text-blue-500 hover:underline">👁️View</a>
                                </td>
                            </tr>
                        ))}
                </tbody>
